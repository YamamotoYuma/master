...
if ( 'post' === get_post_type() ) {
	$post_taxonomy = 'category'; // デフォルトタクソノミー取得.
} else {
	$post_taxonomy = get_post_type() . 'cat'; // カスタムタクソノミー取得.
}
...
<section class="ly_sect__single ly_sect__pager">
	...
	>
		...
		<div class="bl_pager prev">
			<a href="<?php echo esc_url( get_the_permalink( $prevpost->ID ) ); ?>" class="bl_pager_layer"></a>
			...
			<div class="bl_pager_inner cont">
				...
				<h5>
					...
					if ( mb_strlen( $prevpost->post_title ) > $count ) {
						...
						echo esc_html( $prevtitle . '…' );
					} else {
						echo esc_html( $prevpost->post_title );
					}
					?>
				</h5>
				<time> <?php echo esc_html( get_the_time( 'Y年n月j日', $prevpost->ID ) ); ?></time>
			</div>
			<!-- /.bl_pager_inner -->
		</div>
		<!-- /.bl_pager.prev -->
		...
		<div class="bl_pager next">
			<a href="<?php echo esc_url( get_the_permalink( $nextpost->ID ) ); ?>" class="bl_pager_layer"></a>
			<div class="bl_pager_inner thumb">
			...
			<div class="bl_pager_inner cont">
				...
				<h5>
					...
					if ( mb_strlen( $nextpost->post_title ) > $count ) {
						...
						echo esc_html( $nexttitle . '…' );
					} else {
						echo esc_html( $nextpost->post_title );
					}
					?>
				</h5>
				<time> <?php echo esc_html( get_the_time( 'Y年n月j日', $nextpost->ID ) ); ?></time>
			</div>
			<!-- /.bl_pager_inner -->
		</div>
		<!-- /.bl_pager.next -->
