...
if ( is_attachment() ) {
	...
	*/
	...
	if ( 'post' !== $post_type ) {
		...
		// カスタム投稿タイプ名の表示.
		echo '<li>' .
			'<span class="bl_breadcrumb_sep">></span><a href="' . esc_url( $post_type_link ) .'">' .
	...
	if ( false !== $terms ) {
		...
		if ( 0 !== $term->parent ) {
			...
			foreach ( $parent_array as $parent_id ) {
				...
				echo '<li>' .
					'<span class="bl_breadcrumb_sep">></span><a href="' . esc_url( $parent_link ) . ' "> ' .
					'<span>' . esc_html( $parent_name ) . '</span>' .
					'</a>' .
					'</li>';
			}
		}
		...
		// 最下層のタームを表示.
		echo '<li>' .
			'<span class="bl_breadcrumb_sep">></span><a href="' . esc_url( $term_link ) . '">' .
